...
void loop() {
  ...
  if (uidIndex != -1) {
    Serial.print("Authorized Card Detected. Name: ");
    Serial.println(names[uidIndex]);  // Print the name corresponding to the UID
    Serial.println("【Door OPEN】");
    // Reset newScanCount when a new card is scanned
    newScanCount = 0;  // Reset count for new card scan
    ...
    // Check IR sensor for 10 seconds
    while (millis() - entryStartTime < 5000) {
      sensorValue = analogRead(SENSOR_PIN);
      if (sensorValue < threshold) {
        totalCount++;  // Increase total count for the entire system
        newScanCount++;  // Increase count for current scan
        Serial.print("Person Entered. Total Count: ");
        Serial.println(totalCount);
        Serial.print("Total: ");
        Serial.println(newScanCount);  // Print the new count for the current scan
        delay(1000);  // Wait 1 second before counting again to avoid rapid multiple detections
      }
    }
    ...
    lcd.setCursor(0, 1); // Display "Unknown Card"
